...
void print_tree(char* basepath, unsigned int offset) {
    ...
    for(unsigned int itemcur = 0; (item = readdir(directory)) != NULL; itemcur++) {
        ...
        if(item->d_type == DT_DIR) {
            printf("\033[94m\033[1m%s/ %d/%d\n\033[0m", item->d_name, itemcur, nitems);
            strcpy(path, basepath);
            strcat(path, "/");
            strcat(path, item->d_name);
            print_tree(path, offset + PADDING);
        } else {
            printf("%s %d/%d\n", item->d_name, itemcur, nitems);
        }
    }
    closedir(directory);
}
